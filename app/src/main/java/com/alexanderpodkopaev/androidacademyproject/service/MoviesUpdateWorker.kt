...
class MoviesUpdateWorker(context: Context, workerParameters: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        if (moviesFromDB.isNotEmpty() && moviesFromNetwork.isNotEmpty()) findNewMostPopularAndSendNotification(moviesFromDB, moviesFromNetwork)
    ...
    private fun findNewMostPopularAndSendNotification(moviesFromDB: List<Movie>, moviesFromNetwork: List<Movie>) {
        val diff = findNewFilms(moviesFromDB, moviesFromNetwork)
        if (diff.isNotEmpty()) {
            val mostPopularMovie = findMostPopular(diff)
            if (mostPopularMovie != null) {
                sendNotification(mostPopularMovie)
            }
        }
    }
    ...
    private fun sendNotification(mostPopularMovie: Movie) {
